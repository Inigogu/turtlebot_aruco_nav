...
class KalmanFilter:
	...
	def __init__(self):
		...
		self.V_k_min_1 = np.array([0.001, 0.001, 0.0003])
	...
	def ekf(self, Z_k, Q_k, X_k_min_1, P_k_min_1):
		...
		# Predict the state estimate at time k
		X_k = np.dot(self.A_k_min_1, X_k_min_1) + np.dot(self.getB(X_k_min_1[2], self.dk), self.U_k_min_1) + self.V_k_min_1

		# Predict state covariance
		P_k = np.dot(np.dot(self.A_k_min_1, P_k_min_1), self.A_k_min_1.T) + Q_k

		# 2- Correct

		# Difference between sensor and predicted state
		Y_k = Z_k - np.dot(self.H_k, X_k) + self.W_k

		# Residual covariance
		S_k = np.dot(np.dot(self.H_k, P_k), self.H_k.T) + self.R_k

		# Kalman gain
		K_k = np.dot(np.dot(P_k, self.H_k.T), np.linalg.pinv(S_k))

		# Update state estimate
		X_k = X_k + np.dot(K_k, Y_k)

		# Update state covariance
		P_k = P_k - np.dot(np.dot(K_k, self.H_k), P_k)
	...
	def cmd_vel_callback(self, msg):
		...
		self.U_k_min_1[0] = msg.linear.x*0.001 
		self.U_k_min_1[1] = msg.angular.z*0.001
